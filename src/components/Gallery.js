...
class Gallery extends Component {
  ...
  componentDidMount() {
    ...
    console.info(sDom.scrollWidth, sDom.scrollHeight)
    ...
    this.stars.forEach(div => {
      let speed = 0.2 + Math.random()
      let thisDistance = 800 + Math.random() * 300
      div.style.transformOrigin = '0 0 ' + thisDistance + 'px'
      div.style.transform = 'translate3d(0, 0, -' + thisDistance + 'px) rotateY(' + Math.random() * 360 + 'deg) rotateX(' +
        Math.random() * -50 + 'deg) scale(' + speed + ', ' + speed + ')'
    })
  }
  ...
  renderStars() {
    let stars = []
    for (let i = 0; i < this.state.starsCount; i++) {
      stars.push(<div className={'star'} key={`star-${i}`} ref={d => this.stars.push(d)}/>)
    }
